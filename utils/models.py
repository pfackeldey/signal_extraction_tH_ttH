...
class VBFHHFormula:
    ...
    def calculatecoeffients(self):
        ...
        ##############################################
        CV, C2V, kl, a, b, c, iab, iac, ibc, s1, s2, s3, s4, s5, s6 = symbols(
            "CV C2V kl a b c iab iac ibc s1 s2 s3 s4 s5 s6"
        )
        ...
        self.coeffs = (
            c.transpose() * Minv
        )  # coeffs * s is the sigma, accessing per component gives each sample scaling
...
class HHModel(PhysicsModel):
    ...
    def check_validity_ggf(self, ggf_sample_list):
        if len(ggf_sample_list) != 3:
            raise RuntimeError(
                "%s : malformed GGF input sample list - expect 3 samples" % self.name
            )
    ...
    def check_validity_vbf(self, vbf_sample_list):
        if len(vbf_sample_list) != 6:
            raise RuntimeError(
                "%s : malformed VBF input sample list - expect 6 samples" % self.name
            )
    ...
    def getYieldScale(self, bin, process):
        ...
        if len(imatched_ggf) + len(imatched_vbf) != 1:
            print "[ERROR] : in HH model named", self.name, "there are", len(
                imatched_ggf
            ), "GGF name matches and", len(imatched_vbf), "VBF name matches"
            raise RuntimeError(
    ...
    def done(self):
        ...
        for key, val in scalings.items():
            if len(val) > 1:
                ...
                raise RuntimeError(
                    "HHModel : coudl not uniquely match the scaling to the process"
                )
        ...
        for k, i in self.scalingMap.items():
            ...
            if i[0][1] == "GGF":
                ...
                raise RuntimeError(
                    "HHModel : unrecognised type %s - should never happen" % i[0][1]
                )
...
HHdefault = HHModel(
    ggf_sample_list=GGF_sample_list, vbf_sample_list=VBF_sample_list, name="HHdefault"
)
